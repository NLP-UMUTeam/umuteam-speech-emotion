...
class ClassificationHead(nn.Module):
    ...
    def __init__(self, config, num_extra_dims):
        super().__init__()
        # Aquí ahora estamos usando solo audio (768)
        total_dims = 768
...
def get_sentence_embedding(text):
    # (Opcional) si quieres seguir usando BERT para embeddings de texto
    result = tokenizer(
        text,
        truncation=True,
        padding="max_length",
    ...
    with torch.no_grad():
        outputs = bert_model(**result).last_hidden_state[0, 0, :].cpu().tolist()
    return outputs
