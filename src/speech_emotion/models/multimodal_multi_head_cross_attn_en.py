...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, hidden_bert, hidden_wav2vec):
        out = torch.cat([h(hidden_bert, hidden_wav2vec) for h in self.heads], dim=-1)
...
class CustomAudioClassificationAttn(Wav2Vec2BertForSequenceClassification):
    ...
    def forward(
        ...
        # pooled_output: audio, sentence_embedding: texto
        attn_output = self.mult_attn_model(sentence_embedding, pooled_output)
